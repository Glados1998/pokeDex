...
const PokemonDetail = () => {
    ...
    const buildEvolutionChain = async (chain , arr = []) => {
        ...
        arr.push ( {
            name : chain.species.name , id : data.id , image : data.sprites.other.dream_world.front_default
        } );
    ...
    useEffect ( () => {
        Axios.get ( `https://pokeapi.co/api/v2/pokemon/${id}` )
            .then ( response => {
                ...
                // Get evolution chain and flavor_text_entries
                Axios.get ( response.data.species.url )
                    .then ( speciesResponse => {
                        ...
                        const englishFlavorTextEntries = flavorTextEntries.filter ( entry => entry.language.name === 'en' );
                        if ( englishFlavorTextEntries.length > 0 ) {
                            setFlavorText ( englishFlavorTextEntries[0].flavor_text );
                        }

                        // Fetch learnable moves
                        const movePromises = response.data.moves.map ( move => Axios.get ( move.move.url ) );
                                ...
                                <div className="pokemon_body">
                                    <div className="pokemon_body_left">
                                        <div className="pokemon_body_left_header">
                                            ...
                                            <span
                                                className="pokemon_body_left_header_order">Number: {pokemon.order}</span>
                                        </div>
                                    ...
                                    <div className="pokemon_body_right">
                                        <div className="pokemon_body_right_flavor-text">
                                            <p>{flavorText}</p>
                                        </div>

                                        <div className="pokemon_body_right_details">
                                            <div className="pokemon_body_right_details_height-weight">
                                                <b>Height</b> <br/> <span> {pokemon.height} m</span>
                                                <b>Weight</b> <br/> <span> {pokemon.weight} kg</span>
                                            </div>
                                        ...
                                        <div className="pokemon_body_right_evolution">
                                            ...
                                            <div className="pokemon_body_right_evolution_pokemon">
                                                {evolutionChain && evolutionChain.map ( (pokemon , index) => (
                                                    <div className="pokemon_body_right_evolution_pokemon_list"
                                                         key={index}>
                                                        <h5>{pokemon.name}</h5>
